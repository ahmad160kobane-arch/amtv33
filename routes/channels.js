...
router.get('/', (req, res) => {
  try {
    ...
    const countRow = db.prepare(`SELECT COUNT(*) as total FROM channels ${where}`).get(...params);
...
router.get('/groups', (req, res) => {
  const groups = db.prepare('SELECT DISTINCT group_name FROM channels WHERE is_enabled = 1 ORDER BY group_name').all();
  res.json({ groups: groups.map(g => g.group_name) });
});

// GET /api/channels/:id - Get single channel (بدون كشف رابط IPTV)
router.get('/:id', (req, res) => {
  const ch = db.prepare('SELECT id, name, group_name, logo_url, is_enabled, sort_order, created_at FROM channels WHERE id = ?').get(req.params.id);
...
router.get('/stream/:id', requireAuth, requirePremium, async (req, res) => {
  const ch = db.prepare('SELECT id, name, logo_url, stream_url FROM channels WHERE id = ? AND is_enabled = 1').get(req.params.id);
