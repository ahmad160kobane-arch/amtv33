...
import { fetchLuluList, fetchFreeChannels, LuluItem, FreeChannel } from '@/constants/api';
  ...
  const load = useCallback(async () => {
    try {
      await Promise.all([
        ...
        fetchLuluList({ type: 'movie', page: 1, search: 'أطفال' }).then(d => setKidsMovies((d.items || []).slice(0, 16))).catch(() => {}),
        fetchLuluList({ type: 'series', page: 1, search: 'أطفال' }).then(d => setKidsSeries((d.items || []).slice(0, 16))).catch(() => {}),
      ]);
    } finally {
      setLoading(false);
    }
