      ...
      <div className="relative w-full aspect-[2/3] rounded-xl overflow-hidden bg-light-input dark:bg-dark-input">
        {item.poster && !imgError ? (
          <img
            src={item.poster}
            alt={item.title}
            className={`w-full h-full object-cover transition-transform duration-300 group-hover:scale-105 ${(!loading && !isPremium) ? "brightness-75" : ""}`}
        ...
        {/* Type badge */}
        <div
          className={`absolute top-1.5 right-1.5 px-1 py-0.5 rounded text-[8px] sm:text-[9px] font-bold ${
        ...
        {/* Rating badge */}
        {item.rating && (
          <div className="absolute top-1.5 left-1.5 flex items-center gap-0.5 bg-black/60 px-1 py-0.5 rounded">
            <svg
            ...
            <span className="text-[8px] sm:text-[9px] text-white font-bold">
              {item.rating}
            </span>
          </div>
        )}
