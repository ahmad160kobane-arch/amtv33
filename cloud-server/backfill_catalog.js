/**
 * backfill_catalog.js - يُشغَّل على VPS مباشرة
 * تحديث السجلات بدون بيانات TMDB
 */
'use strict';
...
function extractYear(name='') {
  const m = name.match(/\((\d{4})\)\s*$/) || name.match(/[-–]\s*(\d{4})\s*$/);
  return m ? m[1] : '';
}
...
function cleanTitle(name=''){
  return name
    ...
    .replace(/VOD\s*\d*/i,'')
    .replace(/\s*\(\d{4}\)\s*$/,'')      // إزالة (1990) في النهاية
    .replace(/\s*[-–]\s*\d{4}\s*$/,'')   // إزالة - 1990 في النهاية
    .replace(/\s*-\s*(The Movie|Le Film|El Pelicula)\s*$/i,'') // إزالة "- The Movie"
    .replace(/\s{2,}/g,' ').trim();
}
...
async function tmdbSearch(title, type) {
  ...
  // جرّب بعدة أشكال للعنوان: كامل، بدون "-"، أول كلمة قبل "-"
  const variants = [
    cleanName,
    cleanName.replace(/\s*[-–]\s*.+$/,'').trim(),  // قبل أول "-": "Halfaouine"
    cleanName.split(/[-–]/)[0].trim(),
  ].filter((v,i,a) => v && a.indexOf(v) === i); // إزالة التكرار والفراغ

  for (const variant of variants) {
    for (const lang of ['ar','en-US']) {
      const params = {api_key:TMDB_KEY, query:variant, language:lang};
      if (yearFromTitle) {
        if (searchType === 'tv') params.first_air_date_year = yearFromTitle;
        else params.year = yearFromTitle;
      }
      ...
      if (!results.length) {
        // جرّب بدون سنة
        if (yearFromTitle) {
          const p2 = new URLSearchParams({api_key:TMDB_KEY, query:variant, language:lang});
          const r2 = await httpGet('https://api.themoviedb.org/3'+ep+'?'+p2, 12000).catch(()=>null);
          const d2 = parseJson(r2?.body);
          if (d2?.results?.length) { results.push(...d2.results); }
        }
...
(async () => {
  const db = require('./db');
  const rows = await db.prepare('SELECT id, title, vod_type, file_code FROM lulu_catalog WHERE (tmdb_id IS NULL OR poster IS NULL OR poster = $1) ORDER BY uploaded_at DESC').all('');
  ...
  for (const row of rows) {
    ...
    try {
      await db.prepare(
        'UPDATE lulu_catalog SET title = COALESCE(NULLIF(?, $1), title), tmdb_id = COALESCE(tmdb_id, ?), poster = COALESCE(NULLIF(poster, $1), ?), backdrop = COALESCE(NULLIF(backdrop, $1), ?), plot = COALESCE(NULLIF(plot, $1), ?), year = COALESCE(NULLIF(year, $1), ?), rating = COALESCE(NULLIF(rating, $1), ?), genres = COALESCE(NULLIF(genres, $1), ?), cast_list = COALESCE(NULLIF(cast_list, $1), ?), director = COALESCE(NULLIF(director, $1), ?), country = COALESCE(NULLIF(country, $1), ?), runtime = COALESCE(NULLIF(runtime, $1), ?), imdb_id = COALESCE(NULLIF(imdb_id, $1), ?), updated_at = ? WHERE id = ?'
      ...
      if (row.file_code) {
        ...
        const luluRes = await httpGet('https://api.lulustream.com/api/file/edit?'+p, 15000).catch(()=>null);
        const luluData = parseJson(luluRes?.body);
        console.log('  LuluStream update:', luluData?.msg || 'no response');
      }

      console.log('  ✅', bestTitle, '| poster:', !!tmdb.poster, '| plot:', tmdb.plot ? tmdb.plot.slice(0,40)+'...' : 'none');
      updated++;
    } catch(e) {
