...
const MAX_UPSTREAM = 0; // 0 = unlimited — HLS proxy (xtream-proxy) is preferred path
...
class LiveProxy {
  ...
  async streamToClient(channelId, sourceUrl, req, res) {
    ...
    // Multi-channel: no artificial limit on upstream connections
    // HLS proxy (xtream-proxy) is the preferred path for Xtream channels
