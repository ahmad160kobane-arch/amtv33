...
export default function HomeScreen() {
  ...
  const headerTranslateY = clampedScroll.interpolate({
    ...
    outputRange: [0, -totalHeader],
    extrapolate: "clamp",
  });
  const headerOpacity = clampedScroll.interpolate({
    inputRange: [0, HEADER_H * 0.8],
    outputRange: [1, 0],
    extrapolate: "clamp",
  });
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setTvShows(homeData.latestSeries || []);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <AppLogo size="sm" />
        <TouchableOpacity
          onPress={() => router.push("/allcontent" as any)}
          hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
        >
      ...
      >
        {loading && heroItems.length === 0 ? (
          ...
          <HeroSlider items={heroItems} onItemPress={handleVodPress} />
        ) : null}
