...
import { fetchChannels, fetchChannelGroups, Channel } from '@/constants/Api';
...
export default function LiveTabScreen() {
  ...
  const loadData = useCallback(async (reset = true) => {
    try {
      ...
      if (reset) {
        ...
        const groups = await fetchChannelGroups();
  ...
  const renderChannel = useCallback(({ item }: { item: Channel }) => (
    <TouchableOpacity
      ...
      onPress={() => router.push({ pathname: '/player', params: { channelId: item.id, title: item.name } })}
      activeOpacity={0.75}
    >
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { backgroundColor: colors.headerBackground }]}>
        <View style={styles.headerCenter}>
          <Ionicons name="radio" size={20} color={Colors.brand.primary} />
          ...
          ListEmptyComponent={
            !loading ? (
              <View style={styles.empty}>
                ...
                <Text style={[styles.emptySubText, { color: colors.textSecondary }]}>تأكد من تحميل القنوات من لوحة التحكم</Text>
              </View>
            ) : null
          }
        />
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingHorizontal: 16, paddingVertical: 14, direction: 'rtl',
  },
  headerCenter: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  headerTitle: { fontFamily: Colors.fonts.bold, fontSize: 20, textAlign: 'right' },
  headerCount: { fontFamily: Colors.fonts.regular, fontSize: 13 },
  searchContainer: {
