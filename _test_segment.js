...
c.on('ready', () => {
  const cmd = `curl -s -m 30 -o /dev/null -w "HTTP:%{http_code} SIZE:%{size_download} TYPE:%{content_type}" "http://localhost:8090/proxy/live/1017030/seg/http%3A%2F%2F185.191.124.204%3A2095%2Fmypro2025%2F82736475687819901262%2F2127?did=test-device-123&st=${stToken}" -H "Authorization: Bearer ${authToken}" 2>&1`;
  c.exec(cmd, (e, s) => {
    let o = '';
    s.on('data', d => o += d);
