  ...
  try {
    ...
    c.innerHTML = `
      ...
      <div class="panel"><div class="panel-header"><h3>إضافة حساب IPTV</h3></div><div class="panel-body">
        ...
        <div class="form-group"><label><input type="checkbox" id="new-acc-upload"> حساب تحميل فقط (لا يُستخدم للبث)</label></div>
        <div style="border:1px solid var(--border);border-radius:8px;padding:12px;margin:12px 0;background:rgba(99,102,241,0.05)">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px"><span style="font-size:18px">🌐</span><strong>إعدادات VPN / Proxy</strong><label style="margin-right:auto;font-size:13px"><input type="checkbox" id="new-acc-proxy-on"> تفعيل</label></div>
          <div class="form-group"><label>نوع الـ Proxy</label><select class="form-control" id="new-acc-proxy-type"><option value="none">بدون</option><option value="http">HTTP Proxy</option><option value="socks5">SOCKS5 Proxy</option><option value="shadowsocks">Shadowsocks</option><option value="mtproto">MTProto Proxy</option></select></div>
          <div class="form-row"><div class="form-group"><label>السيرفر</label><input class="form-control" id="new-acc-proxy-server" dir="ltr" placeholder="178.104.72.162"></div><div class="form-group"><label>المنفذ (Port)</label><input class="form-control" id="new-acc-proxy-port" dir="ltr" placeholder="443"></div></div>
          <div class="form-group"><label>السر / Secret</label><input class="form-control" id="new-acc-proxy-secret" dir="ltr" placeholder="dd104462821249bd7ac519130220c25d09"></div>
          <div class="form-row"><div class="form-group"><label>المنفذ المحلي</label><input class="form-control" id="new-acc-proxy-local" dir="ltr" placeholder="تلقائي"></div><div class="form-group"><label><input type="checkbox" id="new-acc-proxy-auto" checked> تشغيل تلقائي عند إعادة تشغيل VPS</label></div></div>
        </div>
      ...
      <div class="panel"><div class="panel-header"><h3>الحسابات (${_iptvAccounts.length})</h3></div><div class="panel-body no-pad">
        ${_iptvAccounts.length===0?'<div class="empty-state"><div class="empty-icon">📡</div><p>لا توجد حسابات</p></div>':_iptvAccounts.map(acc=>{
          ...
          const uploadBadge=acc.is_upload_account?'<span style="background:var(--primary);color:#fff;padding:2px 8px;border-radius:4px;font-size:11px;margin-right:6px">📥 تحميل</span>':'';
          return `<div class="iptv-account-card"><div class="acc-header"><div class="acc-info"><strong>${uploadBadge}${esc(acc.name||'حساب #'+acc.id)}</strong><span class="acc-meta" dir="ltr">${esc(acc.server_url)} — ${esc(acc.username)} — ${accChs.length} قناة ${proxyStatus}</span></div>
...
async function addIPTVAccount(){const btn=document.getElementById('add-acc-btn');btn.disabled=true;btn.textContent='جاري...';try{const name=document.getElementById('new-acc-name').value.trim(),server_url=document.getElementById('new-acc-server').value.trim(),username=document.getElementById('new-acc-user').value.trim(),password=document.getElementById('new-acc-pass').value.trim();const is_upload_account=document.getElementById('new-acc-upload')?.checked||false;const proxy_enabled=document.getElementById('new-acc-proxy-on')?.checked||false;const proxy_type=document.getElementById('new-acc-proxy-type')?.value||'none';const proxy_server=document.getElementById('new-acc-proxy-server')?.value.trim()||'';const proxy_port=parseInt(document.getElementById('new-acc-proxy-port')?.value)||0;const proxy_secret=document.getElementById('new-acc-proxy-secret')?.value.trim()||'';const proxy_local_port=parseInt(document.getElementById('new-acc-proxy-local')?.value)||0;const proxy_auto_start=document.getElementById('new-acc-proxy-auto')?.checked!==false;if(!server_url||!username||!password)throw new Error('البيانات مطلوبة');await cloudApi('/api/admin/iptv-accounts',{method:'POST',body:JSON.stringify({name,server_url,username,password,is_upload_account,proxy_enabled,proxy_type,proxy_server,proxy_port,proxy_secret,proxy_local_port,proxy_auto_start})});toast('تم الإضافة');renderPage('iptv');}catch(e){toast(e.message,'error');}btn.disabled=false;btn.textContent='+ إضافة';}
...
async function luluLoadIptvAccounts() {
  try{const d=await cloudApi('/api/lulu-upload/iptv-accounts');LS.iptvAccounts=d.accounts||[];const sel=document.getElementById('lsel-iptv');if(!sel)return;const uploadAccs=LS.iptvAccounts.filter(a=>a.is_upload_account);const streamAccs=LS.iptvAccounts.filter(a=>!a.is_upload_account);let opts='<option value="">الافتراضي</option>';if(uploadAccs.length)opts+=`<optgroup label="📥 حسابات التحميل">${uploadAccs.map(a=>`<option value="${a.id}">📥 ${esc(a.name||a.server_url)}</option>`).join('')}</optgroup>`;if(streamAccs.length)opts+=`<optgroup label="📡 حسابات البث">${streamAccs.map(a=>`<option value="${a.id}">${esc(a.name||a.server_url)}</option>`).join('')}</optgroup>`;sel.innerHTML=opts;}catch{}
}
