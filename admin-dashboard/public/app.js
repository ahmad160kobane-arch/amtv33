...
async function addIPTVAccount(){const btn=document.getElementById('add-acc-btn');btn.disabled=true;btn.textContent='جاري...';try{const name=document.getElementById('new-acc-name').value.trim(),server_url=document.getElementById('new-acc-server').value.trim(),username=document.getElementById('new-acc-user').value.trim(),password=document.getElementById('new-acc-pass').value.trim();const is_upload_account=document.getElementById('new-acc-upload')?.checked||false;if(!server_url||!username||!password)throw new Error('البيانات مطلوبة');await cloudApi('/api/admin/iptv-accounts',{method:'POST',body:JSON.stringify({name,server_url,username,password,is_upload_account})});toast('تم الإضافة');renderPage('iptv');}catch(e){toast(e.message,'error');}btn.disabled=false;btn.textContent='+ إضافة';}
